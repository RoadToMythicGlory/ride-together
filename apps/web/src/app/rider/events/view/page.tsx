...
function RiderEventDetail() {
  ...
  const [event, setEvent] = useState<any>(); const [error, setError] = useState(''); const [loading, setLoading] = useState(false);
  const load = () => getEvent(id).then(setEvent).catch((e) => setError(e.message)); useEffect(() => { if (id) load(); }, [id]);
...
export default function RiderEventDetailPage() {
  return <Suspense fallback={<main className="min-h-[100svh] bg-bg" />}><RiderEventDetail /></Suspense>;
}
